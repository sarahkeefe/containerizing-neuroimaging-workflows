...
# Batch container launching script example - Docker
#=================================================================
# Script: batch_container_launch_template_docker.sh
# Author: Sarah Keefe
# Last Updated: 2023-07-11
# 
# Purpose: This script is intended as a template for you to batch run
#      a Docker container that you build locally. Follow the 
#      instructions in the online documentation and in the comments
#      within the script to integrate your container launch command
#      into this script.
#
# Usage: ./batch_container_launch_template.sh list.csv scans_dir 
#		freesurfers_dir output_dir
...
cat ${input_file} | while IFS=, read -r session_label dti_scan_id fi_threshold_value gradient_value; do
	# Output which session we are on to the console
	...
	# base filename
	#-----------------
	# Get the full file path of the first NIFTI file in the folder
	echo "Checking for NIFTI file for session ${session_label}, scan ID ${dti_scan_id} in the scans folder."

	# Construct the name of the expected DTI scan folder based on the path and DTI scan ID
	# We expect there to be a folder named for the dti scan ID in the named session label subfolder
	# of the scans directory
	...
	# Check whether we found a NIFTI file with that name
	...
	# and bvec files
	#-----------------
	# Use the NIFTI filename to construct the bval and bvec file names
	echo "Getting filenames of .bval and .bvec files - assuming they are in the same folder as the NIFTI file with the same file basenames."

	# Get the base file name of the NIFTI file
	# Instead of /path/to/file.nii.gz, we want file.nii.gz
	nifti_filename=`basename ${nifti_filepath}`

	# Construct the filenames of the bvec and bval files to use in the script
	# Replace .nii.gz or .nii file extension with correct extension for bvec or bval file
	bvec_filename=`echo ${nifti_filename} | sed 's/\.nii\.gz/\.bvec/g; s/\.nii/\.bvec/g'`
	bval_filename=`echo ${nifti_filename} | sed 's/\.nii\.gz/\.bval/g; s/\.nii/\.bval/g'`

	echo "Using NIFTI filename: ${nifti_filename}."
	echo "Using bvec filename: ${bvec_filename}."
	echo "Using bval filename: ${bval_filename}."
	...
	docker_cmd="docker run -d -v ${scans_dir}:/scans -v ${freesurfers_dir}/freesurfers:/freesurfers -v ${output_dir}:/output container_tutorial:1.0-dev /usr/local/bin/run_workflow.sh /scans /freesurfers /output ${session_label} ${dti_scan_id} ${nifti_filename} ${bvec_filename} ${bval_filename} ${fi_threshold_value} ${gradient_value} > ${log_filepath}"

	# Output the Docker command to the console
	echo ${docker_cmd}

	# Use bash "eval" to run the Docker command
	eval ${docker_cmd}

	# Done with this row in the CSV
	echo "Docker container launched for session label=${session_label}, DTI scan ID=${dti_scan_id}"
done

# After looping through the entire CSV file, the script has completed
